...
RC IndexScanOperator::open()
{
  ...
  //类型判断
  AttrType left_attr=left_cell_.attr_type();
  AttrType right_attr=right_cell_.attr_type();
  AttrType compare_attr=left_attr==AttrType::UNDEFINED?right_attr:left_attr;
  if(index_->field_meta().type()!=compare_attr)
  return RC::INTERNAL;
